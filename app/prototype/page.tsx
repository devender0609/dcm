...
interface RecommendationResult {
  ...
  benefitWithSurgery: number; // 0–100
  riskText: string;
  benefitText: string;
  approachProbs: Record<Approach, number>; // 0–1
...
function mockRecommend(form: FormState): RecommendationResult {
  ...
  if (severity === "moderate") surgeryScore += 30;
  if (longDuration) surgeryScore += 15;
  if (highRiskMRI) surgeryScore += 15;
  if (form.gait === "Yes") surgeryScore += 10;

  const riskWithout = Math.min(95, Math.max(5, surgeryScore + 20));
  ...
  let surgeryRecommended: boolean;
  if (surgeryScore < 25) {
    recommendationLabel =
      "Non-operative trial reasonable with close follow-up and structured surveillance.";
    surgeryRecommended = false;
  } else if (surgeryScore < 55) {
    recommendationLabel =
      "Consider surgery; shared decision-making and careful follow-up recommended.";
  ...
  if (form.plannedLevels <= 2 && form.canalRatio !== ">60%") {
    anterior = 0.55;
    posterior = 0.35;
    circumferential = 0.1;
  }

  if (form.plannedLevels >= 4 || form.t2Signal === "multilevel") {
    posterior = 0.6;
    anterior = 0.25;
    circumferential = 0.15;
  }

  if (form.opll === "Yes" && form.canalRatio === ">60%") {
    circumferential = 0.45;
    posterior = 0.4;
    anterior = 0.15;
  }

  const approachProbs: Record<Approach, number> = {
    anterior,
    posterior,
    circumferential,
  };
  ...
  const bestProb = approachProbs[bestKey];
  const secondProb = approachProbs[secondKey];

  let uncertainty: "low" | "moderate" | "high";
  const diff = bestProb - secondProb;
  if (diff >= 0.2) uncertainty = "low";
  else if (diff >= 0.1) uncertainty = "moderate";
  else uncertainty = "high";

  let riskText: string;
  if (!surgeryRecommended) {
    riskText =
      "Low short-term neurological progression risk based on current severity and imaging, but structured follow-up is essential.";
  } else if (severity === "mild") {
    riskText =
      "Mild DCM with long duration and/or MRI risk markers; there is a meaningful risk of neurological progression without decompression.";
  } else if (severity === "moderate") {
    riskText =
      "Moderate DCM with symptomatic cord compression. Published series suggest substantial risk of deterioration without surgery.";
  } else {
    riskText =
      "Severe myelopathy with significant cord compromise. Natural history data show high probability of further irreversible decline without decompression.";
  }

  const benefitText =
    "Estimated chance of achieving clinically meaningful mJOA improvement (MCID) based on severity, duration, imaging surrogates, and comorbidity patterns.";

  return {
    surgeryRecommended,
    recommendationLabel,
    riskWithoutSurgery: riskWithout,
    benefitWithSurgery: benefit,
    riskText,
    benefitText,
    approachProbs,
    bestApproach: surgeryRecommended ? bestKey : "none",
    uncertainty,
  };
}
                    ...
                    <div className="bg-rose-50 border border-rose-100 rounded-2xl px-4 py-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="font-semibold text-rose-700">
                          {result.riskWithoutSurgery.toFixed(0)}%
                        </span>
                      </div>
                    ...
                    <div className="bg-emerald-50 border border-emerald-100 rounded-2xl px-4 py-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="font-semibold text-emerald-700">
                          {result.benefitWithSurgery.toFixed(0)}%
                        </span>
                      </div>
